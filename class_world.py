...
class World:
    ...
    def __init__(self):
        ...
        self.herbivore_avg_age = 100 # in seconds
        self.herbivore_age_std_dev = 7
        self.herbivore_min_age_to_reproduce = 20
        ...
        self.selected_herbivore_nn_output = None #obama
    ...
    def get_simulation_state(self):
        ...
        return {
            "world": {
                ...
                "show_raycast": bool(self.show_raycast),
            },
            "plants": plants,
    ...
    def get_animal_stats(self, species, index):
        if species == "herbivore":
                ...
                stats = {
                        ...
                        "reproduction_progress": float(self.herbivore_reproduction_timers[index] / max(self.herbivore_gestation_time, 1)),
...
################## ----------------------------------- HEBIVORES ----------------------------------------------- ####################
    def update_herbivores(self,dt): # master function that includes all the rest
        ...
        self.herbivores_process_NN() # push neural network inputs to the NNs, get movement parameters out
    ...
    def herbivores_perceive(self):
        ...
        self.herbivore_nn_inputs[alive_indices,self.herbivore_num_external_infos:self.herbivore_num_external_infos+2] = np.stack((self.herbivore_satiety[alive_indices] / self.herbivore_max_satiety, self.herbivore_speeds[alive_indices] / self.max_speed), axis=1)
    ...
    def spawn_herbivore(self, how_many_to_spawn, parent_index=-1):
            ...
            if spawn_count > 0: # if there are actually free slots to spawn AND new herbivores that need spawning, then spawn
                ...
                if parent_index < 0: # if parent index is less than 0, it means we are spawning randomly (not from a parent), so we will spawn in random positions and with random brains
                    ...
                    for parent_param, child_param in zip(parent_brain.parameters(),
                                                        child_brain.parameters()):

                        diff = child_param.data - parent_param.data

                        total_diff += torch.sum(diff * diff).item()
                        total_params += diff.numel()

                    brain_distance = np.sqrt(total_diff / total_params)  
